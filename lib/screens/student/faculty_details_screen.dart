...
class FacultyDetailsScreen extends StatelessWidget {
  ...
  const FacultyDetailsScreen({super.key, required this.name, required this.isStudent});
  ...
  // Add this helper method to the FacultyDetailsScreen class
  ...
  // Update the _buildInfoCard method to use _formatSubjects
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildPhoneRow(
                '', details['parentsContact'] ?? ''),
            
            if (isStudent) _buildInfoRow('Class', details['class'] ?? 'N/A'),
            if (!isStudent) ...[
              _buildInfoRow('Subject', _formatSubjects(details['subject'])),
              const SizedBox(height: 16),
